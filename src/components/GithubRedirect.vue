...
export default{
    ...
    methods:{
        async sendCodeToServer(code,state){
            const response = await axios.post("http://localhost:8080/member/github/doLogin", {code, state});
            const token = response.data.token;

            console.log('백엔드로부터 받은 응답 전체:', response);
            console.log('백엔드 응답 데이터(login info):', response.data);
